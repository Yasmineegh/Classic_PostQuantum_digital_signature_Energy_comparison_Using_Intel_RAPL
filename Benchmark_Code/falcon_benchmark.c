...
int main(int argc, char *argv[]) {
    if (argc < 2) {
        log_printf("Usage: %s <Falcon_Variant>\n", argv[0]);
        log_printf("Available variants: Falcon-512, Falcon-1024\n");
        return 1;
    }
    const char *variant = argv[1];

    if (strcmp(variant, "Falcon-512") != 0 && strcmp(variant, "Falcon-1024") != 0) {
        log_printf("ERROR: Unknown Falcon variant: %s\n", variant);
        return 1;
    }
    
    const char* oqs_variant_name;
    if (strcmp(variant, "Falcon-512") == 0) oqs_variant_name = OQS_SIG_alg_falcon_512;
    if (strcmp(variant, "Falcon-1024") == 0) oqs_variant_name = OQS_SIG_alg_falcon_1024;

    OQS_init();

    crypto_operations_t falcon_ops = {
        .name = variant,
        .short_name = "falcon",
    ...
    int result = run_complete_benchmark(&falcon_ops, oqs_variant_name);
    OQS_destroy();
    return result;
}
