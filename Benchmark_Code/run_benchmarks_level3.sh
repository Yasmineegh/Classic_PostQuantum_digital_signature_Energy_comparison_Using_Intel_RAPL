...
# RSA, ECDSA, Dilithium, and SPHINCS+ at NIST Security Level 3
#
# Security Level: Level 3 (~192-bit equivalent)
# - RSA: 7680-bit (with PSS padding, SHA-384)
# - ECDSA: P-384 curve (SHA-384)
# - Dilithium: Dilithium3 (SHAKE256)
# - SPHINCS+: SPHINCS+-SHA2-192f (SHA-256)
# - SPHINCS+: SPHINCS+-SHA2-192s (SHA-256)
#
# Each algorithm runs 5 times in random order with 300s cooling between runs
# Benchmarks run on isolated cores 1-2
#
# USAGE: sudo ./run_benchmarks_level3.sh
...
# Algorithm configurations at NIST Security Level 3 (~192-bit)
declare -A ALGORITHMS=(
    ["rsa"]="7680"
    ["ecdsa"]="P-384"
    ["dilithium"]="Dilithium3"
    ["sphincs_f"]="SPHINCS+-SHA2-192f"
    ["sphincs_s"]="SPHINCS+-SHA2-192s"
)
...
check_root() {
    if [ "$EUID" -ne 0 ]; then
        ...
        echo "Usage: sudo ./run_benchmarks_level3.sh"
        exit 1
    fi
}
...
compile_all() {
    ...
    if check_liboqs; then
        ...
        warning "Only RSA and ECDSA will be benchmarked."
    fi
    ...
    for algo in "${!ALGORITHMS[@]}"; do
        ...
        if [[ "$algo" =~ ^(dilithium|sphincs_f|sphincs_s) ]] && [ "$has_liboqs" = false ]; then
...
run_all_benchmarks() {
    ...
    info "Algorithms: RSA-7680, ECDSA P-384, Dilithium3, SPHINCS+-SHA2-192f, SPHINCS+-SHA2-192s"
...
- RSA: 7680-bit keys with PSS padding, SHA-384
- ECDSA: P-384 curve (NIST curve), SHA-384
- Dilithium: Dilithium3 (Level 3 security), SHAKE256
- SPHINCS+: SPHINCS+-SHA2-192f (fast variant), SHA-256
- SPHINCS+: SPHINCS+-SHA2-192s (small variant), SHA-256
...
Analysis Notes:
==============
- All algorithms tested at NIST Security Level 3 (~192-bit equivalent)
...
- This security level is suitable for higher-security applications and longer-term protection
- RSA-7680 provides roughly Level 3 classical security (higher cost than Level 1)
...
- RSA: SHA-384 (matched to security level)
- ECDSA: SHA-384 (matched to curve)
- Dilithium: SHAKE256 (post-quantum standard)
- SPHINCS+-192f: SHA-256 (fast variant)
- SPHINCS+-192s: SHA-256 (small variant, optimized for signature size)
...
verify_binaries() {
    ...
    local binaries=("rsa_benchmark" "ecdsa_benchmark" "dilithium_benchmark" "sphincs_benchmark")
...
main() {
    log "Starting Multi-Algorithm Cryptographic Benchmark Suite"
    log "======================================================"
    info "Target security level: NIST Level 3 (~192-bit equivalent)"
    info "Algorithms: 5 (RSA, ECDSA, Dilithium, SPHINCS+-SHA2-192f, SPHINCS+-SHA2-192s)"
    info "Execution: 5 runs per algorithm in randomized order (25 total runs)"
    info "CPU Affinity: Cores ${CPU_CORES} (isolated)"
    echo
