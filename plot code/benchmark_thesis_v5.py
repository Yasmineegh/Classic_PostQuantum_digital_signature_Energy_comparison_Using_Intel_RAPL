#!/usr/bin/env python3
"""
benchmark_thesis_v5.py

What you asked:
1) Throughput: ONE figure that includes BOTH signing + verification, while remaining readable.
   - Uses grouped vertical bars per algorithm (Signing vs Verification).
   - Produces BOTH linear + log versions.

2) Energy: keep Package and Cores separate, but put Signing + Verification in ONE figure for each domain.
   - Two figures per domain (Package, Cores): each includes Signing vs Verification grouped bars.
   - Produces BOTH linear + log versions.

Also:
- Value labels on bars (default ON, can disable)
- Prevent cut titles/labels: constrained_layout + bbox_inches="tight"
- Auto figure sizing + wrapped x tick labels for long algorithm names
- Filters invalid (<=0) means for log-scale plots (log axes cannot show non-positive)

Usage (Windows):
  py benchmark_thesis_v5.py --input .\logs --out .\out --pattern "*.txt"

Disable value labels:
  py benchmark_thesis_v5.py --input .\logs --out .\out --pattern "*.txt" --no-label-values
"""
...
def parse_log_text(text: str, file_name: str) -> Optional[RunMetrics]:
    ...
    def _parse_dynamic_block(domain_label: str) -> Tuple[float, float]:
        ...
        return (_to_float(ms.group(1)) if ms else math.nan,
...
def _auto_figsize(algs: List[str]) -> Tuple[float, float]:
    ...
    width = max(9.0, 1.0 * n + 3.0)
    width = min(width, 26.0)
    height = max(5.2, min(9.0, 4.4 + 0.04 * maxlen))
    return (width, height)
...
def plot_grouped_bars(
    ...
    fig, ax = plt.subplots(figsize=figsize, dpi=250, constrained_layout=True)
    ...
    for j, (means, sds, name) in enumerate(series):
        ...
        ax.bar(xs, means, yerr=sds, capsize=4, width=bar_w, label=name)

        if label_values:
            for xi, val in zip(xs, means):
                if not math.isfinite(val):
                    continue
                if log_scale:
                    ax.text(xi, val * 1.05, f"{val:.3g}", ha="center", va="bottom", fontsize=9)
                else:
    ...
    for ext in [".pdf", ".svg"]:
        fig.savefig(out_base.with_suffix(ext), bbox_inches="tight", pad_inches=0.25)
    fig.savefig(out_base.with_suffix(".png"), dpi=600, bbox_inches="tight", pad_inches=0.25)
    plt.close(fig)
...
def main():
    ...
    if not _HAVE_SCIPY:
        print("NOTE: SciPy not found. 95% CI uses normal approximation (±1.96*SE). "
    ...
    # Throughput: signing + verification in one figure, both scales
    ...
    # Energy: Package (sign+verify) in one figure, both scales
    make_grouped_pair_scales(
        df_summary,
    ...
    # Energy: Cores (sign+verify) in one figure, both scales
    make_grouped_pair_scales(
        df_summary,
    ...
    print("Created grouped Signing vs Verification figures in BOTH linear + log scales for throughput and energy.")
